  ...
  return (
    <section id="services" className="section" aria-labelledby="services-heading">
      <div className="container">
        ...
        <StaggerContainer baseDelay={100} staggerDelay={100} className="grid md:grid-cols-2 lg:grid-cols-4 gap-[24px]">
          {services.map((service, index) => (
            <ScrollReveal key={service.id} delay={index * 100}>
              <Card variant="interactive" padding="lg" className="h-full flex flex-col">
                ...
                <h3 className="text-card-title font-semibold tracking-tight text-text mb-4">{service.title}</h3>
                <p className="text-body-sm text-text-muted mb-6 flex-1">{service.description}</p>
